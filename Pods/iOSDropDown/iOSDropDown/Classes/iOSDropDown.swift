...
extension DropDown: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let selectedImage = self.imageArray[self.selectedIndex!]
        ...
        if let selected = optionArray.index(where: {$0 == selectedText}) {
         ...
         if let selectedImg = optionImageArray.index(where: {$0 == selectedImage}) {
            if let id = optionIds?[selectedImg] {
                didSelectCompletion(selectedImage, selectedImg , id )
            }else{
                didSelectCompletion(selectedImage, selectedImg , 0)
            }
